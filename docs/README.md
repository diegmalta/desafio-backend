...
| [security-privacy.md](security-privacy.md) | HMAC, JWT, isolamento por cidadão, dados sensíveis, notas para produção |
...
## Conformidade com o desenho (leitura crítica)

Esta secção resume uma avaliação honesta; não substitui revisão humana nem auditoria.
...
CPF não é guardado em claro: usa-se fingerprint com `CPF_PEPPER`. O payload WebSocket não inclui CPF. Listagens e `GET /notifications/:id` não vazam existência de notas de outros cidadãos (404 genérico onde aplicável). Pontos a ter em conta em produção: `CheckOrigin` no WebSocket está permissivo (`true`) — adequado para laboratório; em browsers expostos à Internet deve restringir-se origens. Segredos vêm de ambiente; manifests K8s de exemplo usam credenciais de desenvolvimento.
...
**Onboarding**  
Com `README`, `.env.example`, `docker compose` e este diretório `docs/`, deve ser possível subir o projeto e percorrer os fluxos sem perguntas ad hoc; integrações externas opcionais (`CHAMADOS_API_BASE_URL`, etc.) continuam a exigir contexto de ambiente.

**Decisões sem “resposta certa”**  
Schema relacional, nomes de rotas (`/notifications` vs prefixo versionado), e uso de Redis só como fan-out (sem replay no WS) são escolhas documentadas; o importante é consistência interna — que o código e estes documentos mantêm.
